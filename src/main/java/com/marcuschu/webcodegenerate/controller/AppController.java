...
import com.marcuschu.webcodegenerate.annotation.RateLimit;
...
import com.marcuschu.webcodegenerate.model.request.app.*;
import com.marcuschu.webcodegenerate.model.vo.AppVO;
import com.marcuschu.webcodegenerate.service.ProjectDownloadService;
...
public class AppController {
    ...
    public Flux<ServerSentEvent<String>> chatToGenCode(@RequestParam Long appId, @RequestParam String message, @RequestParam(required = false) Boolean agent, HttpServletRequest request) {
        // 参数校验
        ...
        if (agent) {
            contentFlux = appService.chatToGenCode(appId, message, loginUser, agent);
        } else {
    ...
    @Cacheable(
            value = "good_app_page",
            key = "T(com.marcuschu.webcodegenerate.utils.CacheKeyUtils).generateKey(#appQueryRequest)",
